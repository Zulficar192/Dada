...
class MainWindow(QMainWindow):
    ...
    def start_encryption(self):
        ...
        # 获取密钥
        keys = []
        for i in range(self.encryption_levels.value()):
            key = self.key_inputs[i].text()
            if not key:
                QMessageBox.warning(self, "警告", f"请输入第{i+1}层密钥！")
                return
            keys.append(key)
    ...
    def start_decryption(self):
        ...
        # 获取密钥
        keys = []
        for i in range(self.decryption_levels.value()):
        ...
        try:
            ...
            if self_destruct_triggered:
                # 更新文件元数据
                updated_file_data = self.encryptor.update_metadata(file_data, updated_metadata)
